...
Pour commencer il sera plus facile d'apprendre l'assembleur sur une architecture 32 bits, c'est à dire que les registres concernés seront du type _eax_, avec _e_ pour _Extended_ (contrairement aux architectures 64 bits utilisant des registres de type _rax_, avec _r_ pour _Re-extended_). 
...
Une alternative peut être l'utilisation de l'outil en ligne [godbolt](https://godbolt.org/) qui regroupe différentes versions de compilateurs. Afin de suivre les exemples étudiées, il faut _activer_ plusieurs options comme sur l'image ce-dessus :
![Godbolt_gcc7.3_32bits_intel](https://github.com/Sharpforce/Reverse-Engineering/blob/master/.images/01.%20Ressources/godbolt_gcc_7.3_intel_m32.png)
