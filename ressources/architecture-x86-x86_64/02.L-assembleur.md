...
Instruction permettant de charger un offset d'une adresse mémoire au sein du registre _EAX_ (LEA pour Load Effective Address). Attention à bien faire attention ici, ce n'est pas la valeur contenue à l'emplacement _[source-offset]_ qui est chargée, mais bien l'adresse _[source-offset]_ :
```asm
